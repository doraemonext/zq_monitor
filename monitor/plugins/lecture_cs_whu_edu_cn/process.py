...
class Plugin(PluginProcessor):
    ...
    def get_item_list(self):
        return self.get_soup().find('div', attrs={'id': 'container'}).find_all('dd')
    ...
    def get_postdate(self, item):
        return unicode(item.select('i')[0].contents[0])

    def get_content(self, url):
        return unicode(self.get_content_soup(url).find('dd', attrs={'class': 'info'}))
