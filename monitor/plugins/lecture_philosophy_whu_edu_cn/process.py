...
class Plugin(PluginProcessor):
    ...
    def get_item_list(self):
        return self.get_soup().find('div', attrs={'class': 'list'}).find_all('li')
    ...
    def get_postdate(self, item):
        return unicode(item.find('div', attrs={'class': 'date'}).contents[0])

    def get_content(self, url):
        return unicode(self.get_content_soup(url).find('div', attrs={'class': 'news_content'}))
