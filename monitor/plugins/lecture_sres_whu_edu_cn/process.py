...
class Plugin(PluginProcessor):
    ...
    def get_item_list(self):
        return self.get_soup().find_all('td', class_='k3', attrs={'width': '348'})
    ...
    def get_postdate(self, item):
        return unicode(item.find_next('td').find_next('td').find_next('td').contents[0])

    def get_content(self, url):
        return unicode(self.get_content_soup(url).find('div', attrs={'class': 'jl_cons'}))
