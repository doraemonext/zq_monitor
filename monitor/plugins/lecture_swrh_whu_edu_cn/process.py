...
class Plugin(PluginProcessor):
    ...
    def get_item_list(self):
        return self.get_soup().find('div', attrs={'id': 'bd'}).find_all('li')

    def get_title(self, item):
        first = item.find('a').find('b')
        if first is None:
    ...
    def get_content(self, url):
        return unicode(self.get_content_soup(url).find('div', attrs={'class': 'content layout'}))
