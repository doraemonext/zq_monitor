...
class Plugin(PluginProcessor):
    ...
    def get_item_list(self):
        return self.get_soup().find('div', attrs={'id': 'xx1'}).find_all('div', attrs={'id': 'dj4'})
    ...
    def get_postdate(self, item):
        return unicode(item.find_next('div', attrs={'id': 'time1'}).contents[0])

    def get_content(self, url):
        soup = self.get_content_soup(url)
        return unicode(soup.find('div', attrs={'id': 'nrz'}))
