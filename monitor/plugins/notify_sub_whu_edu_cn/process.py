...
class Plugin(PluginProcessor):
    ...
    def get_item_list(self):
        return self.get_soup().find('div', attrs={'class': 'mar_10'}).find_all('li')

    def get_title(self, item):
        return unicode(item.find('a').contents[0]).strip()

    def get_url(self, item):
        return urljoin(urljoin(self.url, '/protect/'), item.find('a')['href'])
    ...
    def get_content(self, url):
        ...
        return unicode(soup.find('div', attrs={'id': 'endtext'}))
